def solve(input):
    ...
    for line in input.split("\n"):
        parts = line.split("-")
        sectorId = int(parts[-1][:3])
        checksum = parts[-1][4:9]
        occurences = {}
        for letter in "".join(parts[:-1]):
            if not letter in occurences:
                occurences[letter] = 0
            occurences[letter] += 1
        # Sort by occurences, then by alphabetic order
        if checksum == "".join([v[0] for v in sorted(occurences.iteritems(), key=lambda (k, v): (-v, k))][:5]):
            sectorSum += sectorId
    print "DONE", sectorSum
