import re
def checkSequence(dest, line):
    currentSequence = ""
    for letter in line:
        currentSequence += letter
        seq = currentSequence[-4:]
        if len(seq) == 4 and seq[0] != seq[1] and seq[:2] == seq[2:][::-1]:
            dest.append(seq)
...
def solve(input):
    ...
    for line in input.split("\n"):
        abbas = []
        hyperabbas = []
        supers = re.findall(r"(\w+\[|\]\w+)", line)
        for super in supers:
            checkSequence(abbas, super)
        hypers = re.findall(r"\[(\w+)", line)
        for hyper in hypers:
            checkSequence(hyperabbas, hyper)

        if not len(hyperabbas) and len(abbas) > 0:
            numValidIps += 1
